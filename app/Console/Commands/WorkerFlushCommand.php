...
class WorkerFlushCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = "worker:flush
      ";
     ...
     * @var string
     */
    protected $description = "Searches for buried jobs in all the queues and deletes them";
    ...
    public function handle()
    {
        ...
        foreach($pheanstalk->listTubes() as $tube) {
		    ...
		    $buriedJob = $pheanstalk->peekBuried($tube);
                    $pheanstalk->delete($buriedJob);
                    $this->info("Job with ID {$buriedJob->getId()} deleted from the queue");
                }
            }
        }
